...
def main() -> None:
    ...
    parser.add_argument('--language', type=str, default='English', help='Language for official track names (e.g., English, Traditional Chinese)')
    ...
    settings = Settings()
    service = CleanerService(prompt_path=args.prompt_path, settings=settings, to_new_dir=args.to_new_dir)
    for album in os.listdir(args.base_path):
        album_path = os.path.join(args.base_path, album)
        if os.path.isdir(album_path):
            print(f"\nProcessing: {album}")
            service.clean_album(album_path, language=args.language)
